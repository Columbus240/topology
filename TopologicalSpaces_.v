...
Program Definition discrete_topology (X : Type) : TopologicalSpace :=
  {| point_set := X; open := (fun _ => True) |}.

Program Definition indiscrete_topology (X : Type) : TopologicalSpace :=
  {| point_set := X;
     open := (fun A => (A = Empty_set) \/ A = Full_set);
  |}.
Next Obligation.
  destruct (classic (exists x, In (FamilyUnion F) x)).
  - destruct H0. destruct H0.
    ...
    + right.
      ...
      red; intros; split; red; intros; try constructor.
      exists S; auto. subst. assumption.
  - pose (not_ex_all_not _ _ H0).
    left.
    apply Extensionality_Ensembles.
    red; intros; split; red; intros.
    + apply n in H1. contradiction.
    + contradiction.
Qed.
Next Obligation.
  destruct H; destruct H0; subst; auto with sets.
  right. apply Extensionality_Ensembles.
  red; intros; split; red; intros; constructor; assumption.
Qed.
