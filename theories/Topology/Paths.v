...
Proof.
  ...
  - apply not_empty_Inhabited.
    ...
    intros ?.
    apply Complement_injective in H1.
    intuition.
...
*)
Lemma order_topology_subspace {X : Type} (R : relation X) (A : Ensemble X) (Rord : order R) (Rcmp : forall x y, R x y \/ R y x) :
  order_convex R A ->
...
Proof.
  intros.
  apply Extensionality_Ensembles; split; red; intros.
  - red in H0. rewrite subspace_open_char in H0.
    destruct H0 as [U [?HU]]. subst.
    destruct HU.
    ...
    constructor.
    red; intros. inversion H1; subst; clear H1.
    Require Import FiniteIntersections.
    destruct H2.
    apply H0 in H1. clear H0.
    induction H1.
    + rewrite inverse_image_full. constructor.
    + inversion H0; subst; clear H0.
      * destruct H2.
        destruct H0. destruct H0. destruct H0.
        destruct (classic (In A x)).
        -- constructor.
           eassert (inverse_image (@subspace_inc (OrderTopology R) A) [y : X | R y x /\ y <> x] = _).
           2: {
             rewrite H4.
             constructor.
           }
           Unshelve.
           2: { exact (exist _ x H3). }
           extensionality_ensembles.
           ...
           ++ constructor. constructor.
              ...
              split; try assumption.
              intros ?. contradict H5.
              apply subset_eq. simpl.
              assumption.
        -- destruct (order_convex_non_element_is_bound A x Rcmp Rord H H3).
           ++ eassert ((inverse_image _ _) = (Intersection _ _)).
              2: {
                rewrite H5.
                apply intro_intersection.
                - apply intro_S.
                  apply (intro_lower_interval _ _ (exist _ x0 H1)).
                - apply intro_S.
                  apply (intro_upper_interval _ _ (exist _ x0 H1)).
              }
              extensionality_ensembles.
              ** destruct H5.
              ...
              ** destruct H5, H6 as [? _].
                 pose proof (ord_antisym Rord _ _ H6 H5).
                 simpl in *. subst.
                 constructor. constructor.
                 split; assumption.
           ++ replace (inverse_image _ _) with (@Full_set {x | In A x}).
              { constructor. }
              extensionality_ensembles.
              2: { constructor. }
              repeat split.
              ** apply H4. apply (proj2_sig x1).
              ** intros ?. subst.
                 contradict H3.
                 apply (proj2_sig x1).
      * destruct H2.
        ...
        destruct (classic (In A x)).
        -- constructor.
           eassert (inverse_image (@subspace_inc (OrderTopology R) A) [y : X | R x y /\ y <> x] = _).
           2: {
             rewrite H4.
           ...
           ++ constructor. constructor.
              ...
              contradict H5. apply subset_eq. simpl.
              reflexivity.
        -- destruct (order_convex_non_element_is_bound A x Rcmp Rord H H3).
           ++ replace (inverse_image _ _) with (@Full_set {x | In A x}).
              { constructor. }
    ...
    + rewrite inverse_image_intersection.
      apply intro_intersection.
      * apply IHfinite_intersections.
        destruct H2. exists x. destruct H2. destruct H2.
        constructor; assumption.
      * apply IHfinite_intersections0.
        destruct H2. exists x. destruct H2. destruct H2.
        constructor; assumption.
  - red. rewrite subspace_open_char.
    ...
    2: {
      ...
      - rewrite inverse_image_family_union_image.
        apply f_equal.
        extensionality_ensembles.
        + exists (proj1_sig (X0 (exist _ x H1))).
          2: {
            pose proof (proj2_sig (X0 (exist _ x H1))) as [].
            simpl in H3.
            assumption.
          }
          exists (exist _ x H1).
          ...
          reflexivity.
        + subst.
          pose proof (proj2_sig (X0 x)) as [].
          rewrite <- H2.
          apply proj2_sig.
    }
    intros [U HU].
    specialize (H0 _ HU).
    Import IndefiniteDescription.
    ...
    + inversion H0; subst; clear H0.
      * exists [y : X | R y (proj1_sig x) /\ y <> (proj1_sig x)].
        split.
        { rewrite <- family_union_singleton.
          constructor.
          red; intros. inversion H0; subst; clear H0.
          apply intro_S.
          apply intro_lower_interval.
        }
        simpl. extensionality_ensembles.
        -- inversion H0; subst; clear H0.
           constructor. constructor.
           split; try assumption.
           intros ?. apply subset_eq in H0.
           auto.
        -- destruct H0.
           constructor. split; try assumption.
           intros ?. subst. contradiction.
      * exists [y : X | R (proj1_sig x) y /\ y <> (proj1_sig x)].
        split.
        { rewrite <- family_union_singleton.
          constructor.
          red; intros. inversion H0; subst; clear H0.
